...
RLAPI int
rl_save_ktx_to_memory(const char* fileName, void* data, int width, int height, int format, int mipmaps); // Save image data as
                                                                                                         // KTX file
...
static int
get_pixel_data_size(int width, int height, int format);
...
void*
rl_load_dds_from_memory(const unsigned char* file_data, unsigned int file_size, int* width, int* height, int* format, int* mips)
{
            ...
            *height = header->height;
            ...
            else if (((header->ddspf.flags == 0x04) || (header->ddspf.flags == 0x05)) && (header->ddspf.fourcc > 0)) // Compressed
            {
                ...
                if (header->mipmap_count > 1)
                    data_size = header->pitch_or_linear_size * 2;
                else
...
void*
rl_load_pkm_from_memory(const unsigned char* file_data, unsigned int file_size, int* width, int* height, int* format, int* mips)
{
    void* image_data = NULL; // Image data pointer
...
void*
rl_load_ktx_from_memory(const unsigned char* file_data, unsigned int file_size, int* width, int* height, int* format, int* mips)
{
    void* image_data = NULL; // Image data pointer
    ...
    if (file_data_ptr != NULL) {
        ...
        if ((header->id[1] != 'K') || (header->id[2] != 'T') || (header->id[3] != 'X') || (header->id[4] != ' ') ||
            (header->id[5] != '1') || (header->id[6] != '1')) {
...
int
rl_save_ktx(const char* file_name, void* data, int width, int height, int format, int mipmaps)
{
    // KTX file Header (64 bytes)
    // v1.1 - https://www.khronos.org/opengles/sdk/tools/KTX/file_format_spec/
    ...
    typedef struct {
        ...
        unsigned int faces;                   // Cubemap faces, for no-cubemap = 1
        unsigned int mipmap_levels;           // Non-mipmapped textures = 1
        unsigned int key_value_data_size; // Used to encode any arbitrary data...     // KTX 2.0: UInt32 levelOrder - ordering
                                          // of the mipmap levels, usually 0 KTX 2.0: UInt32 supercompressionScheme - 0 (None),
                                          // 1 (Crunch CRN), 2 (Zlib DEFLATE)...
    ...
    if (header.gl_format == -1)
        LOG("WARNING: IMAGE: GL format not supported for KTX export (%i)", header.gl_format);
    else {
    ...
    if (file != NULL) {
        ...
        if (count == 0)
            LOG("WARNING: FILEIO: [%s] Failed to write file", file_name);
        else if (count != data_size)
            LOG("WARNING: FILEIO: [%s] File partially written", file_name);
        else
            LOG("INFO: FILEIO: [%s] File saved successfully", file_name);
        ...
        LOG("WARNING: FILEIO: [%s] Failed to open file", file_name);
...
void*
rl_load_pvr_from_memory(const unsigned char* file_data, unsigned int file_size, int* width, int* height, int* format, int* mips)
{
    void* image_data = NULL; // Image data pointer
    ...
    if (file_data_ptr != NULL) {
        ...
        else if (pvr_version == 52)
            LOG("INFO: IMAGE: PVRv2 format not supported, update your files to PVRv3");
    }
