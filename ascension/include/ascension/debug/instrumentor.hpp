...
#define PROFILE_BEGIN_SESSION(name, filepath) ascension::debug::Instrumentor::get().begin_session(name, filepath) // NOLINT
#define PROFILE_END_SESSION() ascension::debug::Instrumentor::get().end_session()                                 // NOLINT

// NOLINTNEXTLINE
#define PROFILE_SCOPE_LINE_INTERNAL(name, line)                                                                                \
    constexpr auto fixedName##line = ascension::debug::_cleanup_timer_name(name, "__cdecl ");                                  \
    ascension::debug::Instrumentor_Timer timer##line(std::string(fixedName##line.data.data()))
