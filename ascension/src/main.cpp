...
main()
{
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
            ...
            for (size_t i = 0; i < 100; i++) {
                if (bunniesCount < MAX_BUNNIES) {
                    ...
                    bunnies[bunniesCount].color = { static_cast<unsigned char>(GetRandomValue(50, 240)),
        ...
        for (size_t i = 0; i < bunniesCount; i++) {
            ...
            DrawTexture(
                texBunny, static_cast<int>(bunnies[i].position.x), static_cast<int>(bunnies[i].position.y), bunnies[i].color
            );
        }
