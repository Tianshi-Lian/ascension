...
template<typename Char>
void
parse_flags(basic_format_specs<Char>& specs, const Char*& it, const Char* end)
{
    for (; it != end; ++it) {
        switch (*it) {
            case '-':
...
int
parse_header(const Char*& it, const Char* end, basic_format_specs<Char>& specs, GetArg get_arg)
{
    int arg_index = -1;
    Char c = *it;
    if (c >= '0' && c <= '9') {
...
inline auto
vsprintf(const S& fmt, basic_format_args<basic_printf_context_t<type_identity_t<Char>>> args) -> std::basic_string<Char>
{
...
template<typename S, typename Char = char_t<S>>
inline auto
vfprintf(std::FILE* f, const S& fmt, basic_format_args<basic_printf_context_t<type_identity_t<Char>>> args) -> int
{
    basic_memory_buffer<Char> buffer;
    vprintf(buffer, detail::to_string_view(fmt), args);
...
template<typename S, typename Char = char_t<S>>
inline auto
vprintf(const S& fmt, basic_format_args<basic_printf_context_t<type_identity_t<Char>>> args) -> int
{
...
inline auto
printf(const S& fmt, const T&... args) -> int
{
