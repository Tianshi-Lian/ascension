...
#include <cmath>   // Required for: fabsf() [Used in DrawTextureRec()]
#include <cstdio>  // Required for: sprintf() [Used in ExportImageAsCode()]
#include <cstdlib> // Required for: malloc(), free()
#include <cstring> // Required for: strlen() [Used in ImageTextEx()], strcmp() [Used in LoadImageFromMemory()]
...
#include "stb/stb_image.hpp" // Required for: stbi_load_from_file()
...
#include "rl/gputex.hpp" // Required for: rl_load_xxx_from_memory()
...
#include "stb/stb_image_write.hpp" // Required for: stbi_write_*()
#endif

#if defined(SUPPORT_IMAGE_GENERATION)
#define STB_PERLIN_IMPLEMENTATION
#include "stb/stb_perlin.hpp" // Required for: stb_perlin_fbm_noise3
#endif

#define STBIR_MALLOC(size, c) ((void)(c), RL_MALLOC(size))
#define STBIR_FREE(ptr, c) ((void)(c), RL_FREE(ptr))
#define STB_IMAGE_RESIZE_IMPLEMENTATION
#include "stb/stb_image_resize.hpp" // Required for: stbir_resize_uint8() [ImageResize()]

static Vector4*
LoadImageDataNormalized(Image image); // Load pixel data from image as Vector4 array (float normalized)
...
LoadImage(const char* fileName)
{
    ...
    defined(SUPPORT_FILEFORMAT_HDR) || defined(SUPPORT_FILEFORMAT_PNM) || defined(SUPPORT_FILEFORMAT_PSD)
...
Image
LoadImageFromMemory(const char* fileType, const unsigned char* fileData, int dataSize)
{
    Image image = { 0 };
...
IsImageReady(Image image)
{
    return (
        ...
        (image.mipmaps > 0)
    ); // Validate image mipmaps (at least 1 for basic mipmap level)
}
...
ExportImage(Image image, const char* fileName)
{
    ...
    if (IsFileExtension(fileName, ".png")) {
        ...
        unsigned char* fileData = stbi_write_png_to_mem(
            (const unsigned char*)imgData, image.width * channels, image.width, image.height, channels, &dataSize
        );
    ...
    if (false) {
        ...
        channels = 0;
        if (image.format == RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8)
            channels = 3;
        else if (image.format == RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8)
            channels = 4;
        else
...
#define TEXT_BYTES_PER_LINE 20
    ...
    byteCount += sprintf(
        txtData + byteCount, "// ImageAsCode exporter v1.0 - Image pixel data exported as an array of bytes         //\n"
    );
    byteCount += sprintf(
        txtData + byteCount, "//                                                                                    //\n"
    );
    ...
    byteCount += sprintf(
        txtData + byteCount, "#define %s_FORMAT   %i          // raylib internal pixel format\n\n", varFileName, image.format
    );
    ...
    if (success != 0)
        TRACELOG(LOG_INFO, "FILEIO: [%s] Image as code exported successfully", fileName);
    else
        TRACELOG(LOG_WARNING, "FILEIO: [%s] Failed to export image as code", fileName);
...
GenImageColor(int width, int height, Color color)
{
    ...
    Image image = {
        .data = pixels, .width = width, .height = height, .mipmaps = 1, .format = RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8
    };
...
Image
GenImageGradientV(int width, int height, Color top, Color bottom)
{
...
Image
GenImageGradientH(int width, int height, Color left, Color right)
{
    Color* pixels = (Color*)RL_MALLOC(width * height * sizeof(Color));
...
Image
GenImageGradientRadial(int width, int height, float density, Color inner, Color outer)
{
    Color* pixels = (Color*)RL_MALLOC(width * height * sizeof(Color));
...
Image
GenImagePerlinNoise(int width, int height, int offsetX, int offsetY, float scale)
{
    Color* pixels = (Color*)RL_MALLOC(width * height * sizeof(Color));
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            pixels[y * width + x] = (Color){ static_cast<unsigned char>(intensity),
...
void
ImageCrop(Image* image, Rectangle crop)
{
    ...
    if (image->format >= RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
        ...
        for (int y = (int)crop.y; y < (int)(crop.y + crop.height); y++)
        {
            ...
            {
                //memcpy(croppedData + ((y - (int)crop.y)*(int)crop.width + (x - (int)crop.x))*bytesPerPixel, ((unsigned char
        *)image->data) + (y*image->width + x)*bytesPerPixel, bytesPerPixel); for (int i = 0; i < bytesPerPixel; i++)
            ...
            switch (image->format) {
                ...
                case RL_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA: {
                    ...
                    for (int i = 0; i < image->width * image->height * 2; i += 2, k++) {
                        ((unsigned char*)image->data)[i] = (unsigned char
                        )((pixels[k].x * 0.299f + (float)pixels[k].y * 0.587f + (float)pixels[k].z * 0.114f) * 255.0f);
        ...
        if (codepoint == '\n') {
            ...
            if ((codepoint != ' ') && (codepoint != '\t')) {
                ...
                ImageDraw(
                    ...
                    (Rectangle){ 0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height },
                    rec,
                    tint
                );
            }
...
ImageResize(Image* image, int newWidth, int newHeight)
{
    ...
    if ((image->format == RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) || (image->format == RL_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA) ||
        ...
        switch (image->format) {
            case RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
                stbir_resize_uint8(
                    (unsigned char*)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, 1
                );
                break;
            case RL_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA:
                stbir_resize_uint8(
                    (unsigned char*)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, 2
                );
                break;
            case RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8:
                stbir_resize_uint8(
                    (unsigned char*)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, 3
                );
                break;
            case RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8:
                stbir_resize_uint8(
                    (unsigned char*)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, 4
                );
                break;
            default:
                break;
        ...
        stbir_resize_uint8(
            (unsigned char*)pixels, image->width, image->height, 0, (unsigned char*)output, newWidth, newHeight, 0, 4
        );
...
void
ImageResizeCanvas(Image* image, int newWidth, int newHeight, int offsetX, int offsetY, Color fill)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0))
        return;
...
void
ImageToPOT(Image* image, Color fill)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0))
        return;
...
void
ImageAlphaClear(Image* image, Color color, float threshold)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0))
        return;
...
ImageBlurGaussian(Image* image, int blurSize)
{
    ...
    Vector4* pixelsCopy1 = static_cast<Vector4*>(RL_MALLOC((image->height) * (image->width) * sizeof(Vector4)));
    Vector4* pixelsCopy2 = static_cast<Vector4*>(RL_MALLOC((image->height) * (image->width) * sizeof(Vector4)));
...
ImageMipmaps(Image* image)
{
    ...
    if (image->mipmaps < mipCount) {
        ...
        for (int i = 1; i < mipCount; i++) {
            TRACELOGD(
                "IMAGE: Generating mipmap level: %i (%i x %i) - size: %i - offset: 0x%x",
                i,
                mipWidth,
                mipHeight,
                mipSize,
...
void
ImageColorReplace(Image* image, Color color, Color replace)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0))
        return;
...
// NOTE: Threshold is defined as a percentage: 0.0f -> 1.0f
Rectangle
GetImageAlphaBorder(Image image, float threshold)
{
    Rectangle crop = { 0 };
...
void
ImageDrawPixel(Image* dst, int x, int y, Color color)
{
    // Security check to avoid program crash
...
void
ImageDrawPixelV(Image* dst, Vector2 position, Color color)
{
...
void
ImageDrawLine(Image* dst, int startPosX, int startPosY, int endPosX, int endPosY, Color color)
{
...
void
ImageDrawLineV(Image* dst, Vector2 start, Vector2 end, Color color)
{
...
void
ImageDrawCircle(Image* dst, int centerX, int centerY, int radius, Color color)
{
    int x = 0;
    int y = radius;
...
void
ImageDrawCircleLines(Image* dst, int centerX, int centerY, int radius, Color color)
{
    int x = 0;
    int y = radius;
    int decesionParameter = 3 - 2 * radius;
...
void
ImageDrawCircleLinesV(Image* dst, Vector2 center, int radius, Color color)
{
...
void
ImageDrawRectangle(Image* dst, int posX, int posY, int width, int height, Color color)
{
...
void
ImageDrawRectangleV(Image* dst, Vector2 position, Vector2 size, Color color)
{
...
// Draw rectangle within an image
void
ImageDrawRectangleRec(Image* dst, Rectangle rec, Color color)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0))
        return;
...
void
ImageDrawRectangleLines(Image* dst, Rectangle rec, int thick, Color color)
{
    ...
    ImageDrawRectangle(
        dst, (int)(rec.x + rec.width - thick), (int)(rec.y + thick), thick, (int)(rec.height - thick * 2), color
    );
...
ImageDraw(Image* dst, Image src, Rectangle srcRec, Rectangle dstRec, Color tint)
{
    ...
    if (dst->format >= RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
        ...
        // TODO: Support RL_PIXELFORMAT_UNCOMPRESSED_R32, RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32,
        ...
        if ((tint.a == 255) &&
            ((srcPtr->format == RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) ||
             (srcPtr->format == RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8) || (srcPtr->format == RL_PIXELFORMAT_UNCOMPRESSED_R5G6B5)))
            blendRequired = false;
...
ImageDrawText(Image* dst, const char* text, int posX, int posY, int fontSize, Color color)
{
    ...
    ImageDrawTextEx(
        dst, GetFontDefault(), text, position, (float)fontSize, (float)fontSize / 10, color
    ); // WARNING: Module required: rtext
#else
...
LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != RL_CUBEMAP_LAYOUT_AUTO_DETECT) {
        ...
        if (layout == RL_CUBEMAP_LAYOUT_LINE_VERTICAL) {
            ...
            if (layout == RL_CUBEMAP_LAYOUT_LINE_HORIZONTAL)
                for (int i = 0; i < 6; i++)
...
IsTextureReady(Texture2D texture)
{
    ...
    return (
        (texture.id > 0) &&                            // Validate OpenGL id
        (texture.width > 0) && (texture.height > 0) && // Validate texture size
        (texture.format > 0) &&                        // Validate texture pixel format
        (texture.mipmaps > 0)
    ); // Validate texture mipmaps (at least 1 for basic mipmap level)
}
...
{
    return (
        (target.id > 0) &&              // Validate OpenGL id
        IsTextureReady(target.depth) && // Validate FBO depth texture/renderbuffer
        IsTextureReady(target.texture)
    ); // Validate FBO texture
}
...
DrawTextureNPatch(Texture2D texture, NPatchInfo nPatchInfo, Rectangle dest, Vector2 origin, float rotation, Color tint)
{
    if (texture.id > 0) {
        ...
        if (nPatchInfo.layout == RL_NPATCH_THREE_PATCH_HORIZONTAL)
            patchHeight = nPatchInfo.source.height;
        if (nPatchInfo.layout == RL_NPATCH_THREE_PATCH_VERTICAL)
...
Color
ColorBrightness(Color color, float factor)
{
    Color result = color;
...
Color
ColorContrast(Color color, float contrast)
{
    Color result = color;
...
// Get color with alpha applied, alpha goes from 0.0f to 1.0f
Color
ColorAlpha(Color color, float alpha)
{
    if (alpha < 0.0f)
        alpha = 0.0f;
    else if (alpha > 1.0f)
...
#define COLORALPHABLEND_INTEGERS
    ...
    if (src.a == 0)
        ...
        if (out.a > 0) {
            out.r = (unsigned char
            )((((unsigned int)src.r * alpha * 256 + (unsigned int)dst.r * (unsigned int)dst.a * (256 - alpha)) / out.a) >> 8);
            out.g = (unsigned char
            )((((unsigned int)src.g * alpha * 256 + (unsigned int)dst.g * (unsigned int)dst.a * (256 - alpha)) / out.a) >> 8);
            out.b = (unsigned char
            )((((unsigned int)src.b * alpha * 256 + (unsigned int)dst.b * (unsigned int)dst.a * (256 - alpha)) / out.a) >> 8);
        }
    }
#endif
...
Color
GetColor(unsigned int hexValue)
{
    Color color;
...
Color
GetPixelColor(void* srcPtr, int format)
{
    Color color = { 0 };
    ...
    switch (format) {
        case RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
            ...
            break;
        case RL_PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA:
            color = (Color){ ((unsigned char*)srcPtr)[0],
...
void
SetPixelColor(void* dstPtr, Color color, int format)
{
    switch (format) {
        case RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: {
            // NOTE: Calculate grayscale equivalent color
...
static Vector4*
LoadImageDataNormalized(Image image)
{
