...
template<typename CodeUnit>
void
write_codecvt(codecvt_result<CodeUnit>& out, string_view in_buf, const std::locale& loc)
{
...
template<typename Char, typename OutputIt, FMT_ENABLE_IF(!std::is_same<Char, char>::value)>
auto
write(OutputIt out, const std::tm& time, const std::locale& loc, char format, char modifier = 0) -> OutputIt
{
...
FMT_CONSTEXPR inline const char*
get_units()
{
...
inline Int
to_nonnegative_int(T value, Int upper)
{
...
To
fmt_safe_duration_cast(std::chrono::duration<FromRep, FromPeriod> from)
{
    int ec;
...
OutputIt
format_duration_value(OutputIt out, Rep val, int)
{
...
OutputIt
copy_unit(string_view unit, OutputIt out, Char)
{
...
template<typename OutputIt>
OutputIt
copy_unit(string_view unit, OutputIt out, wchar_t)
{
