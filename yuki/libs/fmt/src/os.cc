...
inline unsigned
convert_rwcount(std::size_t count)
{
...
int
detail::utf16_to_utf8::convert(basic_string_view<wchar_t> s)
{
    if (s.size() > INT_MAX)
  ...
  public:
      ...
      , message_(nullptr)
    {
        result_ = FormatMessageW(
            ...
            reinterpret_cast<wchar_t*>(&message_),
            0,
            nullptr
        );
        if (result_ != 0) {
...
std::system_error
vwindows_error(int err_code, string_view format_str, format_args args)
{
...
void
detail::format_windows_error(detail::buffer<char>& out, int error_code, const char* message) noexcept
{
    FMT_TRY
    {
        system_message msg(error_code);
...
int
buffered_file::descriptor() const
{
...
std::size_t
file::read(void* buffer, std::size_t count)
{
    rwresult result = 0;
...
std::size_t
file::write(const void* buffer, std::size_t count)
{
    rwresult result = 0;
...
void
file::dup2(int fd, std::error_code& ec) noexcept
{
    int result = 0;
    FMT_RETRY(result, FMT_POSIX_CALL(dup2(fd_, fd)));
    if (result == -1)
...
void
file::pipe(file& read_end, file& write_end)
{
