 ...
 * Last Modified: 2023-04-24 14:47:03
 * ------------------
...
Sprite_Batch::initialize(u32 screen_width, u32 screen_height, const std::shared_ptr<Shader>& sprite_shader, u32 max_batch_size)
{
    if (m_max_batch_size > 0) {
        yuki::debug::Logger::error("erika", "Sprite_Batch should only be initialized once!");
        return;
    }
...
{
    if (m_batch_active) {
        yuki::debug::Logger::warn("erika", "Sprite_Batch::begin() has already been called. Call Sprite_Batch::end() first.");
        return;
    }
...
{
    if (!m_batch_active) {
        yuki::debug::Logger::warn("erika", "Sprite_Batch::end() has already been called without an active batch.");
        return;
    }
...
{
    if (!m_batch_active) {
        yuki::debug::Logger::warn("erika", "Sprite_Batch::draw() has already been called without an active batch.");
        return;
    }
